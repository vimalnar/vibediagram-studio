...
const StarterScene: ImplementationDefinition["Scene"] = ({
  ...
  useEffect(() => {
    ...
    phaseApi.start({
      from: { phase: 0 },
      to: { phase: starterConfig.metadata.stages.length },
      config: { duration: starterConfig.timelineDurationMs },
      loop,
      onRest: (result) => {
  ...
  useFrame((state, delta) => {
    ...
    if (!manualCamera) {
      const target = new Vector3(...starterConfig.camera.target);
      const distance = starterConfig.camera.distance / safeZoom;
      const azimuth = starterConfig.camera.azimuth;
      const elevation = starterConfig.camera.elevation;
...
export const starterImplementation: ImplementationDefinition = {
  metadata: {
    id: "starter",
    title: starterConfig.metadata.title,
    description: starterConfig.metadata.description,
    intro: starterConfig.metadata.intro,
    tip: starterConfig.metadata.tip,
    stages: starterConfig.metadata.stages,
  },
  theatre: {
    projectId: "StarterExplainer",
    sheetId: "Main",
  },
  defaultCamera: starterConfig.camera,
  Scene: StarterScene,
};
